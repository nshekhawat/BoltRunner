...
export default {
  ...
  *makeMaterials(ctx) {
    ...
    return {
      ...
      crystal: std({ color: 0xa0e0ff, emissive: 0x2080c0, emissiveIntensity: 0.6, roughness: 0.1, transparent: true, opacity: 0.55 }),
  ...
  pickup: { makePickupShell: (ctx, M) => { const { prim: P, THREE } = ctx, g = P.group(); // blue ice-encased cell with frost crystals; snow spirals in
    const shell = new THREE.Mesh(new THREE.IcosahedronGeometry(0.62, 1), M.crystal); shell.userData.noOutline = true; g.add(shell);
    for (let i = 0; i < 6; i++) { const c = P.cone(0.06, 0.28, M.frost, 0, 0, 0, 5); c.position.set(Math.cos(i * 1.05) * 0.62, Math.sin(i * 2.3) * 0.3, Math.sin(i * 1.05) * 0.62); c.lookAt(c.position.clone().multiplyScalar(3)); c.rotateX(Math.PI / 2); c.userData.noOutline = true; g.add(c); }
