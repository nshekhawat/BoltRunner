...
export class Game {
  ...
  update(dt) {
    ...
    const mode = this.state === 'PLAYING' ? (this.rocket ? 'jump' : p.grounded ? (p.ducking ? 'duck' : 'run') : 'jump') : this.state === 'CRASHED' ? 'stumble' : this.state === 'COUNTDOWN' ? 'run' : 'idle';
  ...
  activatePower(kind) {
    ...
    if (kind === 'shield') this.bubble = true; else if (kind === 'magnet') this.obstacles.magnet = true; else if (kind === 'slowmo') this.timeScale = 0.6; else if (kind === 'rocket') this.rocket = true;
    hud.message({ shield: '🛡️ SHIELD', magnet: '🧲 MAGNET', slowmo: '⏳ SLOW-MO', rocket: '🚀 ROCKET!' }[kind], 1.2, true, true); this.emit('power', kind);
  }
  endPower() { if (!this.power) return; const k = this.power.kind; this.power = null; if (k === 'shield') this.bubble = false; if (k === 'magnet') this.obstacles.magnet = false; if (k === 'slowmo' && this.slowT <= 0) this.timeScale = 1; if (k === 'rocket') { this.rocket = false; this.player.vy = 0; } hud.power(null); }
